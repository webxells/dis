...
public class DefaultDescription extends SinglePart<Option> {

    public DefaultDescription(final RootConfiguration.Configuration configuration) {
        super(configuration);
    }

    @Override
    protected String getValue(final Option current) {
        if (current.getName().equals("amount")) {
            var x =1;
        }
        return current.getAnnotation(Default.class)
                .map(Default::value)
                .orElse(null);
    }
