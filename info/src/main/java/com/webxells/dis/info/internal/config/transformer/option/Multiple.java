...
public class Multiple extends SinglePart<Option> {

    public Multiple(final RootConfiguration.Configuration configuration) {
        super(configuration);
    }
    ...
    protected String getValue(final Option current) {
        return Optional.of(
                current.getParameter().isArray() || Collection.class.isAssignableFrom(current.getParameter()))
                .filter(a -> a)
                .map(String::valueOf)
                .orElse(null);
    }
