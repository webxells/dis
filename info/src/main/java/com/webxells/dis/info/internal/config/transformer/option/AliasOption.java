...
public class AliasOption extends SinglePart<Option> {

    public AliasOption(final RootConfiguration.Configuration configuration) {
        super(configuration);
    }

    @Override
    protected String getValue(final Option current) {
        return current.getAnnotation(Alias.class)
                .map(Alias::value)
                .orElse(null);
    }
