...
@Description("Reads http status code of the response")
public class HttpStatusCode implements ContentStrategy{
    @Override
    ...
    public InputStream parseOutputRequest(final Response response) {
        return new ByteArrayInputStream(String.valueOf(response.getStatusCode()).getBytes());
    }
}